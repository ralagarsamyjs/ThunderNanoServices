...
namespace Plugin {
    ...
    private:
        ...
        public:
            ...
            {   
                printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher()->PID<%d><%d>\n", getpid(), getpid());
                if(_parentInterface != nullptr){
                    printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher()->PID<%d><%d>calling _parentInterface->AddRef()\n", getpid(), getpid());
                    _parentInterface->AddRef();
                }
                printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher()->PID<%d><%d>calling Open(Core::infinite)\n", getpid(), getpid());
                Open(Core::infinite);
            }
            ~TokenDispatcher() override
            {
                if(_parentInterface != nullptr){
                    printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher()->PID<%d><%d>calling _parentInterface->Release()\n", getpid(), getpid());
                    _parentInterface->Release();
                }
                printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher()->PID<%d><%d>calling Close(Core::infinite)\n", getpid(), getpid());
                Close(Core::infinite);
            }
        ...
        private:
            ...
            {
                void* result = nullptr;
                printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher::Acquire()->PID<%d><%d>interfaceId<%d>versionId<%d> \n", getpid(), getpid(), interfaceId, versionId);
                if (((versionId == 1) || (versionId == static_cast<uint32_t>(~0))) && ((interfaceId == PluginHost::IAuthenticate::ID) || (interfaceId == Core::IUnknown::ID))) {
                    printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher::Acquire()->PID<%d><%d>calling _parentInterface->AddRef()\n", getpid(), getpid());
                    _parentInterface->AddRef();
                ...
                printf("WPEFramework::Plugin::SecurityAgent::TokenDispatcher::Acquire()->PID<%d><%d> return result<%p>\n", getpid(), getpid(), result);
                return (result);
            }
