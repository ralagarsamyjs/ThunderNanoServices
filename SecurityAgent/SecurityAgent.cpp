...
namespace Plugin {
    ...
    /* virtual */ const string SecurityAgent::Initialize(PluginHost::IShell* service)
    {
        ...
        printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d>webPrefix<%s>callsign<%s>\n", getpid(), gettid(), webPrefix.c_str(), callsign.c_str());
        ...
        printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> InvokeServer Create()\n", getpid(), gettid());
        _engine = Core::ProxyType<RPC::InvokeServer>::Create(&Core::IWorkerPool::Instance());
        printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> _dispatcher.reset()\n", getpid(), gettid());
        _dispatcher.reset(new TokenDispatcher(Core::NodeId(connector.c_str()), service->ProxyStubPath(), this, _engine));

        if (_dispatcher != nullptr) {
            printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> _dispatcher->IsListening()\n", getpid(), gettid());
            if (_dispatcher->IsListening() == false) {
                printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> _dispatcher->reset(nullptr)\n", getpid(), gettid());
                _dispatcher.reset(nullptr);
                printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> _engine.Release()\n", getpid(), gettid());
                _engine.Release();
            } else {
                if (subSystem != nullptr) {
                    ...
                    printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> calling subSystem->Set(PluginHost::ISubSystem::SECURITY)\n", getpid(), gettid());
                    subSystem->Set(PluginHost::ISubSystem::SECURITY, &information);
                    printf("WPEFramework::Plugin::SecurityAgent::Initialize()->PID<%d><%d> calling subSystem->Release()\n", getpid(), gettid());
                    subSystem->Release();
                }
            }
        }
    ...
    /* virtual */ void SecurityAgent::Deinitialize(PluginHost::IShell* service)
    {
        ...
        ASSERT(subSystem != nullptr);
        printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d> \n", getpid(), gettid());
        if (subSystem != nullptr) {
            printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d>calling subSystem->Set(NOT_SECURITY) \n", getpid(), gettid());
            subSystem->Set(PluginHost::ISubSystem::NOT_SECURITY, nullptr);
            printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d>calling subSystem->Release() \n", getpid(), gettid());
            subSystem->Release();
        }
        printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d>calling _acl.Clear() \n", getpid(), gettid());
        _acl.Clear();
        printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d>calling _dispatcher.reset(nullptr) \n", getpid(), gettid());
        _dispatcher.reset(nullptr);

        if (_engine.IsValid()) {
            printf("WPEFramework::Plugin::SecurityAgent::Deinitialize()->PID<%d><%d>calling _engine.Release() \n", getpid(), gettid());
            _engine.Release();
        }
    }
    ...
    /* virtual */ uint32_t SecurityAgent::CreateToken(const uint16_t length, const uint8_t buffer[], string& token)
    {
        ...
        printf("WPEFramework::Plugin::SecurityAgent::CreateToken()->PID<%d><%d>calling newToken->Encode()\n", getpid(), gettid());
    ...
    /* virtual */ PluginHost::ISecurity* SecurityAgent::Officer(const string& token)
    {
        ...
        printf("WPEFramework::Plugin::SecurityAgent::Officer()->PID<%d><%d>calling newToken->Encode()\n", getpid(), gettid());
